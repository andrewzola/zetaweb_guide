...
* также верстка подразумевает переход на необходимую страницу через клик на любую часть области слайда - для этого каждому элементу с классом "carousel\_\_item\_link" требуется присвоить значение аттрибута "href". Элемент с классом "carousel\_\_item\_button" - кнопка, которой тоже нужно указать адрес страницы сайта. 
