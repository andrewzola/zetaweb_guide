...
## Версия 2.6.1.1 от 29.03.2022 <a href="#d0-b2-d0-b5-d1-80-d1-81-d0-b8-d1-8f-2611-d0-be-d1-82-29032022" id="d0-b2-d0-b5-d1-80-d1-81-d0-b8-d1-8f-2611-d0-be-d1-82-29032022"></a>

**Минимальная версия для обновления: 2.6.0.0**

**Главное**

* Это последний релиз в ветке 2.х. Все последующее развитие будет происходить в ветке 3.х
...
* Для справочника "Автомобили клиента" теперь обязательно указание модификации
* Уточнено ценообразования для контрагентов, которые зарегистрированы на сайте по роли пользователя сайт

### Исправлено:&#x20;

* В некоторых случаях не проводился документ оплаты платежной картой при обмене с сайтом
* Получение кроссировок брендов в конфигурации Виртуальный склад ред. 3

### Удалено: ##\#

* Нет.

## Версия 2.6.0.0 от 12.04.2021 #\#

**Минимальная версия для обновления: 2.5.0.0**

**Главное**

* Расширена интеграция с конфигурацией "Виртуальный склад 3"

### Добавлено: ##\#

* Добавлен Web-сервис "ExchangeRMM", добавлена роль "ДоступКВебСервисуРММ". При настройке интеграции с конфигурацией "Виртуальный склад 3" необходимо включить эту роль для пользователя, от имени которого настроена интеграция в виртуальном складе.
* Добавлено переопределяемое событие "ВыполнитьПередВыгрузкой", в этом событии можно переопределить действия которые будут выполнены перед началом выгрузки данных на сайт.
* Добавлено переопределяемое событие "ВыполнитьПередЗагрузкой", в этом событии можно переопределить действия которые будут выполнены перед началом загрузки данных с сайта.

### Изменено: ##\#

* В справочнике "зс_веб_АвтомобилиКлиента" изменены типы реквизитов "ИдентификаторМарки", "ИдентификаторМодели", "Тип" со строки на число.
* Для базовых прав разрешено изменение периодов закрытия заказов покупателей.

### Исправлено: ##\#

* Исправлено поведение отмены проведения, когда проведенный заказ приходит с сайта и не может провестись в базе.
* Исправлена ошибка получения кроссировок брендов в http-сервисе ZetaApi в том случае, если не указан идентификатор эталонного бренда WSG

### Удалено: ##\#

* Нет.

## Версия 2.5.0.0 от 01.06.2020 #\#

**Минимальная версия для обновления: 2.4.11.0**

**Главное**

* Это последняя обновление, которое лицензируется полностью. В дальнейшем лицензирование функциональности будет происходить по отдельным контролам.
* В запросах обмена данными добавлена дата среза, на которую будут выгружаться остатки и цены (для уменьшения объема больших таблиц на сайте).&#x20;

### Добавлено: ##\#

* В форме настроек обмена с сайтом можно указать дату среза обмена (по умолчанию - 1 января 1980 г.
* Обмен кросировками брендов с конфигурацией "Виртуальный склад 3" (с версии 3.0.1.9).

### Изменено: ##\#

* Наименование в URL для характеристик на сайте теперь не включает в себя наименование номенклатуры.
* Для базовых прав разрешено изменение периодов закрытия заказов покупателей.

### Исправлено: ##\#

* Исправлена функция отправки сообщения о множественных ошибках обмена.
* Перенос необработанных брендов виртуального склада в кросс-бренды.
* Исправлено поведение обработки настройки доставки и способов оплаты.
* Устранена возможность указать описание для размещения "заказ поставщику" или "склад поставщика", не указав поставщика.&#x20;
* Устранена возможность указать группу доступности складов для размещения "заказ поставщику" или "склад поставщика", не указав поставщика.
* Исправлен подсчет количества товаров по бренду в виртуальном складе.
