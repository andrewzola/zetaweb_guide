...
### Версия 3.0.1.0 от 15.10.2024

#### Добавлено:

* Новые методы Core для оформления заказа
* Возможность контрола поиска получать данные для вывода информации на сайте через API Core

### Версия 2.6.2.6 (3.0.0.5) от 28.04.2023

#### Добавлено:

* Дополнительное логирование ошибок
* Поиск заказа клиента по его уникальному идентификатору

#### Изменено:

* Оптимизация поиска товаров по тексту

#### Исправлено:

* Отображение групп запчастей каталога ""Лаксимо" для каталогов, не поддерживающих отображение "общего" каталога
* Отображение изображений товаров, полученных из API
* Произвольная ошибка отображения контролов при обновлении страницы в режиме работы нескольких обслуживающих процессов IIS
* Прочие ошибки
...
### Версия 2.6.2.5 (3.0.0.4) от 20.02.2023
...
* Проверка авторизационного cookie-файла при загрузке страниц и AJAX-контролов
* Правила определения AJAX-запросов
* Логирование ошибок приложения
* Оптимизация API (Dapper)

#### Исправлено:

* Ошибка перенаправления запросов на другую страницу, вызванных AJAX-отрисовкой контролов
* Ошибка получения изображений через API
* Ошибка получения описания производителей (брендов) через API
* Ошбика получения типов оплаты и доставки через API

#### Удалено:

* Нет

### Версия 2.6.2.4 (3.0.0.3) от 13.01.2023

#### Добавлено:

* Настройки отключения морфологического поиска и поиска по началу строки в контроле "Поиск. Поиск товаров (уточнение артикула и бренда товара)"
* Получение аналогов товара через API
...
* Ошибка инициализации контролов после AJAX-обновления (NRE)
* Ошибка отображения изображений, полученных через API
* Ошибка получения печатных форм в формате PDF

#### Удалено:

* Нет

### Версия 2.6.2.3 (3.0.0.2) от 30.11.2022

#### Добавлено:

* Ограничение выборки в контролах "Поиск. Предложения группы каталога (API)" и "Поиск. Предложения сопутствующих товаров (API)" по минимальному рейтингу товара
* Метод завершения обмена с настраиваемым количеством размера порции обмена
* Расширенное логирование ошибок в API
...
* Установка meta-тегов страницы в контроле "Поиск. Текущая группа каталога (API)"
* Ошибка каталогов Laximo (поиск модификаций по VIN)
* Ошибка определения доуступных складов при получение прайс-листа через API

#### Удалено:

* Нет

### Версия 2.6.2.2 (3.0.0.1) от 14.11.2022

#### Добавлено:

* Сохранение описания состояния отправки прайс-листа клиенту
* Получение способов доставки и оплаты через API
...
### Версия 2.6.2.1 (3.0.0.0) от 29.09.2022
...
* Оптимизация API (обработка отмены запроса)

#### Исправлено:

* Ошибка перехода к онлайн-оплате заказа через платежную систему "Сбербанк"
* Ошибка переполнения ключа в таблице настроек групп доступности складов

#### Удалено:

* Нет

### Версия 2.6.2.0 от 10.08.2022

#### Добавлено:

* Расширенное логирование ошибок
* Информация о датах добавления, изменения, старой цены и старого количества в строках в корзины

#### Изменено:

* Использование реквизита "Роль" в таблице "Веб-пользовтели" вместо регистра "Роли пользователей"

#### Исправлено:

* Формирование некоторых Url

#### Удалено:

* Нет

### Версия 2.6.1.5 от 06.06.2022
...
* Получение предложений из API 2.0 (ошибка учета группы доступности складов)
* Обработка SEO-перенаправления на сстраницу "404"

#### Удалено:

* Нет

### Версия 2.6.1.4 от 20.05.2022

#### Добавлено:

* Работа с заявками на отмену/возврат товара в контроле "Состояние строк заказов покупателей. Группировки."
* Вывод информации об НДС в контроле "Печатная форма платежного документа"
* Возможность отправки файлов прайс-листов в раздельных почтовых сообщениях
...
* Получение суммы НДС для заказов с "НДС сверху"
* Распределение количества в контроле "Загрузка из Excel. Результат поиска"

#### Удалено:

* Нет

### Версия 2.6.1.1 от 22.12.2021
...
* Оптимизация обмена дарнными (сниженияе уровня блокировок; уменьшение размера пакетов обновляемых данных)
...
* Возмонность перехода к запросам на возврат и отмену строки заказа в контроле "Состояние строк заказов покупателей. Строки."
...
* Пперенос предложений, полученных от WSG, между корзинами клиента
...
* Контрол поиска групп каталога товаров по тексту товаров, расположенных в этих группах.
...
* Формирование прайс-листов (API, оптимизация)
...
* Ошибка отображения специальных html-символов вида &\_\_\_; в html-текстах.
