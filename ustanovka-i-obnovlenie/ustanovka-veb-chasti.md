...
* logs
* Prices
* ReferencesCache и файл log.xml

4. Архивируем выбранные файлы, задаем значимое имя \(пример формата: имя сайта + номер версии приложения + дата архивации\)

5. Переводим сайт в оффлайн для внешних пользователей и поисковых систем. Для этого изменяем имя файла \_app\_offline.htm, убирая первое подчеркивание.   
Теперь сайт будет отображать информацию из этой страницы.

6. Делаем бэкап базы данных сайта средствами SQL. С информацией по формированию бэкапа можно ознакомиться на сайте производителя СУБД.  
...
7. Копируем на веб-сервер дистрибутив нового приложения. Внутри дистрибутива располагаются две папки - CMS и DB.

8. Заменяем файлы приложения, расположенные в каталоге нашего сайта, на файлы, расположенные в папке **CMS.**

9. Переводим сайт в рабочий режим - обратно добавляем первое подчеркивание у файла app\_offline.htm - он снова становится \_app\_offline.htm.

10. Перезапускаем сайт. Открываем главную страницу и дожидаемся её отображения.
